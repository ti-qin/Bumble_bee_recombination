...
HOM_REF_MAX_ALT_FRACTION = 0.10

# A homozygous ALT call must have ALT fraction >= 0.90.
HOM_ALT_MIN_ALT_FRACTION = 0.90
...
# Small INDELs are 1-19 bp because the article specifies <20 bp.
MIN_INDEL_LENGTH = 1
MAX_INDEL_LENGTH = 19
...
def get_family_vcf(
    ...
    return (
        ...
        / (
            ...
            "biallelic.small_indels.PASS.vcf.gz"
        )
    )
...
def classify_indel(
    ...
    return {
        ...
        "indel_length": abs(length_change),
    }
...
# ============================================================
# 7. Scan family-specific PASS INDEL VCFs
# ============================================================
...
for family in families:
    ...
    if not vcf.exists():
        raise FileNotFoundError(
            f"{family} PASS INDEL VCF not found: {vcf}"
        )
    ...
    for line in process.stdout:
        ...
        family_counters[
            "pass_biallelic_indel_sites_examined"
        ] += 1

        vcf_indel_length = abs(len(alt) - len(ref))
        ...
        if not (
            ...
            family_counters[
                "indel_length_filter_failed"
            ] += 1
            continue

        family_counters[
            "small_indel_length_passed"
        ] += 1
        ...
        queen_allele_sequence = (
            ref
            if queen_allele_index == 0
            else alt
        )

        mutant_allele_sequence = (
            ref
            if mutant_allele_index == 0
            else alt
        )

        indel_info = classify_indel(
            queen_allele_sequence,
            mutant_allele_sequence,
        )

        if indel_info is None:
            family_counters[
                "zero_length_change_failed"
            ] += 1
            continue

        candidate_id = (
            f"{family}_DNM_INDEL_"
            f"{chromosome}_{position}_"
            f"{indel_info['mutation_type'].upper()}"
            f"{indel_info['indel_length']}_"
            f"{mutant_sample}"
        )
        ...
        candidate_rows.append(
            {
                ...
                "queen_allele": queen_allele_sequence,
                "mutant_offspring_id": (
                    mutant_sample
                ),
                "mutant_allele_index": (
                ...
                "mutant_allele": mutant_allele_sequence,
                "mutation": (
                    f"{queen_allele_sequence}>{mutant_allele_sequence}"
                ),
                "mutation_type": indel_info["mutation_type"],
                "length_change": indel_info["length_change"],
                "indel_length": indel_info["indel_length"],
                "other_four_offspring_absent": True,
                "n_other_offspring_with_mutant_allele": 0,
                "n_queen_matching_offspring": (
                    len(matching_offspring)
                ),
        ...
        family_counters[
            "preliminary_denovo_indel_candidates"
        ] += 1
...
candidate_columns = [
    ...
    "length_change",
    "indel_length",
    "other_four_offspring_absent",
    "n_other_offspring_with_mutant_allele",
    "n_queen_matching_offspring",
    "matching_offspring_ids",
    "site_QUAL",
    "site_FILTER",
...
combined_candidate_file = (
    OUT_ROOT
    / "all_families.preliminary_denovo_INDEL_candidates.tsv.gz"
)
...
manual_review_file = (
    OUT_ROOT
    / "all_families.denovo_INDEL_manual_review.tsv"
)
...
manual_review_df = candidate_df[
    manual_review_columns
].copy()
manual_review_df["manual_review_decision"] = "PENDING"
manual_review_df["alignment_consistent"] = "PENDING"
manual_review_df["exact_indel_supported"] = "PENDING"
manual_review_df["repeat_or_homopolymer_flag"] = "PENDING"
manual_review_df["nearby_misalignment_flag"] = "PENDING"
manual_review_df["manual_review_notes"] = ""
...
combined_genotype_file = (
    OUT_ROOT
    / "all_families.preliminary_denovo_INDEL_genotypes.long.tsv.gz"
)
...
for family in families:
    ...
    family_tsv = (
        ...
        / (
            f"{family}."
            "preliminary_denovo_INDEL_candidates.tsv.gz"
        )
    )
    ...
    family_bed = (
        ...
        / (
            f"{family}."
            "preliminary_denovo_INDEL_candidates.bed"
        )
    )
    ...
    family_vcf = (
        ...
        / (
            f"{family}."
            "preliminary_denovo_INDEL_candidates.vcf.gz"
        )
    )
...
filter_summary.to_csv(
    SUMMARY_DIR
    / "denovo_INDEL_filter_summary.tsv",
    sep="\t",
    index=False,
)
    ...
    family_summary_rows.append(
        {
            ...
            "n_preliminary_denovo_INDEL_candidates": (
                len(
                    family_candidates
                )
            ),
...
family_summary.to_csv(
    SUMMARY_DIR
    / "denovo_INDEL_candidate_summary.tsv",
    sep="\t",
    index=False,
)
...
parameter_rows = [
    {
        ...
        "value": (
            "family-specific PASS biallelic INDELs"
        ),
        "comment": (
    ...
    {
        "parameter": "MIN_INDEL_LENGTH",
        "value": MIN_INDEL_LENGTH,
        "comment": "Minimum absolute REF-ALT length difference.",
    },
    {
        "parameter": "MAX_INDEL_LENGTH",
        "value": MAX_INDEL_LENGTH,
        "comment": "The article requires small INDELs shorter than 20 bp.",
    },
    {
...
pd.DataFrame(
    ...
    SUMMARY_DIR
    / "denovo_INDEL_calling_parameters.tsv",
    sep="\t",
    index=False,
)
...
print(
    "\nPreliminary de novo INDEL candidate summary:"
)
